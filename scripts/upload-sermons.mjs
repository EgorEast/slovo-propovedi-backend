...
function parseSermonFileName(fileName) {
  ...
  if (main) {
    ...
    // Fail at parse time instead of storing the MP3 in MinIO and only then
    // discovering the POST /sermons 400 (orphan file).
    if (endChapter && moreParts) {
      throw new Error('Диапазон глав несовместим с перечислением стихов через запятую — ссылка на Писание не распознана');
    }
